...
function getMpgs(speed) {
  ...
  for(let i=0; i < data.length; i++) {
    const val = data[i]['0_60'];
    if (val >= (speed - 0.1) || val <= (speed + 0.1)) {
...
function getStats(speed) {
  ...
  // const gaussian = getData(mean, stddev, [10,50], 0.1);
...
function getGaussianData(mean, stddev, range, granularity) {
  const data = [];

  for (let i = range[0]; i<range[1]; i += granularity) {
    const y = (1 / Math.sqrt(2 * Math.PI * stddev * stddev)) * Math.exp(-1 * (i - mean) * (i - mean) /(2 * stddev * stddev))
    data.push(y)
  }
...
function getGaussian(speed, range, granularity) {
  const stats = getStats(speed);
  return getGaussianData(stats.mean, stats.stddev, range, granularity);
}
